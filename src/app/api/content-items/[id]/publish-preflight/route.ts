...
export async function POST(_request: Request, { params }: RouteContext) {
  try {
    ...
    const result = buildPublishPreflightDryRun({
    ...
    return NextResponse.json({ error: safeErrorMessage(error instanceof Error ? error.message : "Publish preflight dry-run failed.", 500) }, { status: 400 });
  }
}
