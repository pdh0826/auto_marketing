...
- `/wizard/daily-brief` exposes `무료 TTS 음성 입히기`, which may call only local `say` or `espeak-ng`.
- Voiceover render writes local-only `voiceover-script.txt`, local audio, `voiceover-render-report.json`, and `video-with-voiceover.mp4` when local TTS and ffmpeg are available.
...
- Publish approval preview for the published item should include `content_status_not_planned` and `content_already_published`.
- Publish approval save for the published item should return HTTP 400 with `content_status_not_planned` and must not create another approval row.
- Publish execution attempt save for the published item should return HTTP 400 with `approval_no_longer_matches_current_state` and `content_status_changed`.
- Guarded live publish for the published item must keep `liveExecutionAttempted=false` unless every gate passes, and in normal smoke should include `content_status_not_planned` and `content_already_published`.
...
- `previewMode=read_only_draft_generation_llm_request_envelope_preview`.
- `dryRunOnly=true`.
- `requestEnvelopeBuiltForPreview=true`.
- `requestEnvelopeStored=false`.
- `requestWouldBeSent=false`.
- `currentSideEffectSummary.requestSentToProvider=false`.
...
- `requestEnvelopePreviewSummary.headerValuesExposed=false`.
- `requestEnvelopePreviewSummary.secretValuesExposed=false`.
- `requestEnvelopePreviewSummary.endpointValueExposed=false`.
- `requestEnvelopePreviewSummary.redactionSummary.rawSecretsIncluded=false`.
- `requestEnvelopePreviewSummary.redactionSummary.rawTokensIncluded=false`.
- `requestEnvelopePreviewSummary.redactionSummary.rawEnvValuesIncluded=false`.
- Forbidden secret/token scan should pass with no response JSON occurrences of dangerous credential strings.
- Non-preview mode such as `dispatch` should include `draft_generation_llm_request_envelope_preview_is_preview_only`.
- Side effects should remain false for DB write, provider health check, provider network call, LLM call, LLM evaluator call, `llm_call_logs` mutation, content mutation, draft Markdown/HTML mutation, Blogger write, publish, schedule, OAuth reconnect, token refresh, publish approval mutation, and publish attempt mutation.
...
- `POST /api/daily-content-plans/draft-generation-readiness` should support `mode=preflight` for plan item `cmqlr1v1y0001iwj2gpv2875r` and linked content item `daily_fixture_cmqlr1v1y0001iwj2gpv2875r`.
- Preflight should return `patchVersion=9F-2F`, `checked=true`, `planItemFound=true`, `fixtureFound=true`, and `linkedFixtureMatchesPlanItem=true`.
- Current linked fixture should return `readinessSummary.structuralReadyForFutureDraftGeneration=true`, `executionReadyForDraftGeneration=false`, and `readinessLevel=structural_ready_but_execution_blocked`.
- Checks should report `fixtureNotPublished=true`, `fixtureNotScheduled=true`, `fixtureHasNoDraftMarkdown=true`, `fixtureHasNoDraftHtml=true`, `planRequiresHumanApproval=true`, `operatorApprovalPersisted=false`, `draftGenerationFlagEnabled=false`, `llmGenerationFlagEnabled=false`, `contentMutationAllowed=false`, `bloggerWriteAllowed=false`, `publishExecutionAllowed=false`, and `scheduledPublishAllowed=false`.
- Blocking reasons should include the absence of persisted operator approval and disabled draft-generation/LLM/content mutation execution.
- Non-preflight mode such as `mode=generate` should be blocked with `draft_generation_readiness_is_preflight_only`, `applyAttempted=false`, `generationAttempted=false`, `applyBlocked=true`, `generationBlocked=true`, `applyOk=false`, `generationOk=false`, and all write/external side-effect flags false.
- `/settings/blogger` should show `초안 생성 준비 점검`, the linked fixture id, structural-ready/execution-blocked status, missing requirements, disabled generation/LLM buttons, and technical side-effect details.
- Post-change DB checks should remain `blog_daily_content_plans_count=1`, `blog_daily_content_plan_items_count=3`, `content_items_count=2`, and publish milestone counts `1 / 1 / 1 / 1 / 22`.
- The linked fixture should remain `status=planned`, `publishedAt=null`, `scheduledAt=null`, empty `draftMarkdown`, and empty `draftHtml`.
- 9F-2F must not rerun 9F-2B apply or 9F-2D apply.
- 9F-2F must not create/update/delete daily plan rows, daily plan item rows, `content_items`, approval rows, publish attempts, or Blogger rows.
...
- The existing migration `20260620000200_add_daily_content_operator_approval_scaffold` should be applied exactly once with `npx prisma migrate deploy` after explicit operator approval.
- No new migration should be created.
- `prisma/schema.prisma` should not be changed in this patch.
- After apply, `npx prisma migrate status` should report `Database schema is up to date!`.
- `blog_daily_content_operator_approvals` and `blog_daily_content_operator_approval_events` should exist in the DB.
- `operator_approvals_count` should remain `0`.
- `operator_approval_events_count` should remain `0`.
- Post-apply DB checks should remain `blog_daily_content_plans_count=1`, `blog_daily_content_plan_items_count=3`, `content_items_count=2`, and publish milestone counts `1 / 1 / 1 / 1 / 22`.
- The linked fixture `daily_fixture_cmqlr1v1y0001iwj2gpv2875r` should remain `planned`, not published, not scheduled, and with empty `draftMarkdown`/`draftHtml`.
- The 9E published baseline should remain unchanged.
- 9F-2J preview after migration apply should report `operatorApprovalTablesExist=true`, `operatorApprovalPersistenceAvailable=true`, `executionAllowed=false`, `operatorApprovalSatisfied=false`, and blocker `operator_approval_missing`.
- 9F-2J preview after migration apply should not include `operator_approval_tables_not_applied`.
- 9F-2I-APPLY must not create approval rows or events.
- 9F-2I-APPLY must not rerun 9F-2B apply or 9F-2D apply.
- 9F-2I-APPLY must not create/update/delete daily plan rows, daily plan item rows, `content_items`, approval rows, publish attempts, Blogger rows, or operation profile rows.
...
- `POST /api/daily-content-plans/draft-generation-final-execution-checklist` should support only `mode=preview`.
- Preview should report `patchVersion=9F-2Q`, `checklistMode=read_only_draft_generation_final_execution_checklist`, `dryRunOnly=true`, `executionAllowed=false`, and `finalDraftGenerationAllowed=false`.
- Preview should report `llmCompletionAttempted=false`, `promptRendered=false`, `providerNetworkCallAttempted=false`, `contentMutationAttempted=false`, and `bloggerWriteAttempted=false`.
- Target summary should show linked fixture `daily_fixture_cmqlr1v1y0001iwj2gpv2875r`, status `planned`, not published, not scheduled, and draft lengths `0 / 0`.
- Operator approval summary should show `operatorApprovalSatisfied=true`.
- Prior gate summary should show dry-run planner, LLM provider readiness, health-check preview, and health-check execution gate are available, while `healthCheckExecutedNow=false` and `providerNetworkCallAttemptedNow=false`.
- Remaining execution blockers should stay `llm_execution_feature_flag_disabled`, `content_mutation_feature_flag_disabled`, `draft_generation_write_feature_flag_disabled`, `confirmation_phrase_missing`, and `idempotency_key_missing`.
- Final checklist should include pass, blocked, and caution items plus an operator runbook and recommended next patch.
- Non-preview modes should be blocked with `draft_generation_final_execution_checklist_is_preview_only`.
- Current side effects should be `dbRead=true` and `envRead=true`; `dbWrite`, `secretValueExposed`, `providerHealthChecked`, `providerNetworkCall`, `promptRendered`, `rawPromptStored`, `llmCompletion`, `llmCall`, `llmCallLogMutation`, `contentItemMutation`, `draftMarkdownMutation`, `draftHtmlMutation`, `bloggerWrite`, `bloggerDraftSave`, `bloggerPublish`, `scheduledPublish`, `oauthReconnect`, `tokenRefresh`, `publishApprovalMutation`, `publishAttemptMutation`, and `externalSend` should be `false`.
- `/settings/blogger` should show `초안 생성 최종 실행 체크리스트` with target, fixture state, approval state, prior gate status, remaining blockers, pass/blocked/caution checklist, operator runbook, side-effect summary, and recommended next patch.
- 9F-2Q should not rerun 9F-2B apply, 9F-2D apply, 9F-2I-APPLY, or 9F-2K approval apply.
- 9F-2Q should not call LLM providers, render prompts, run provider network calls, create `llm_call_logs`, mutate `content_items`, create/update drafts, write to Blogger, publish/schedule, reconnect OAuth, refresh tokens, mutate publish approvals, or mutate publish attempts.
...
## Patch 9F-3D gated LLM dispatch audit artifact persistence 검증

- `POST /api/daily-content-plans/draft-generation-llm-dispatch-artifact-creation` route가 있어야 한다.
- `mode=preview`는 DB read만 수행하고 artifact row를 만들지 않아야 한다.
- feature flag 없이 `mode=apply`를 호출하면 `llm_dispatch_artifact_create_feature_flag_disabled` blocker로 차단되고 row count는 그대로여야 한다.
- `BLOG_DAILY_CONTENT_LLM_DISPATCH_ARTIFACT_CREATE_ENABLED=true`, exact confirmation phrase, idempotency key, candidate artifact, duplicate-artifact gate가 모두 만족되면 artifact row 1건만 생성되어야 한다.
- 생성 row는 `artifactKind=prompt_request_hash_bundle`, `artifactStorageMode=hash_only`, `artifactRedactionStatus=redacted_or_hash_only`, safe bounded preview, `rawSecretStored=false`, `rawTokenStored=false`를 저장해야 한다.
- 같은 target/kind/hash로 다시 apply하면 기존 artifact를 반환하고 추가 row를 만들지 않아야 한다.
...
## Patch 9F-3G LLM dispatch final preflight 검증

- `POST /api/daily-content-plans/draft-generation-llm-dispatch-final-preflight`는 read-only여야 한다.
- Response는 attempt/event/artifact audit readiness, prompt quality, request envelope, provider readiness, health-check readback, idempotency/confirmation policy를 종합해야 한다.
- `dispatchExecutionAllowedInThisPatch=false`, `readyForLlmDispatchExecution=false`여야 한다.
